...
import {
	...
	loadTokenizedAssetPlatformAddr,
} from "../viemUtils";
...
async function main() {
	...
	console.log("Parameters:", verifyParams);
	...
	try {
		...
		const hash = await walletClient.writeContract({
			...
			args: [
				verifyParams.assetId,
				verifyParams.isValid,
				verifyParams.verificationDetails,
			],
		});

		console.log("Asset Verification Transaction sent! Hash:", hash);
